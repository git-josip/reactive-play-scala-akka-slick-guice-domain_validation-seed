...
object CustomSlickMapper
{
  object Postgres
  {
    ...
    {
      PostgresProfile.MappedColumnType.base[T, String](
        { adt => adt.name                                    },
        { dv  => StringUtils.parseEnum[T](dv)(classManifest) }
      )
    }

    implicit val jodaDateTimeColumnType = PostgresProfile.MappedColumnType.base[org.joda.time.DateTime, java.sql.Timestamp](
      { dt => new java.sql.Timestamp(dt.getMillis) },
      { ts => new org.joda.time.DateTime(ts)       }
    )

    implicit val jodaLocalDateColumnType = PostgresProfile.MappedColumnType.base[org.joda.time.LocalDate, java.sql.Date](
      { dt => new java.sql.Date(dt.toDateTimeAtStartOfDay.getMillis) },
      { d => new LocalDate(d.getTime)       }
    )

    implicit val jsValueColumnType = PostgresProfile.MappedColumnType.base[JsValue, String](
      { jsv =>  jsv.toString()  },
      { sv => Json.parse(sv)    }
    )

    implicit val jodaPeriodTimeColumnType = PostgresProfile.MappedColumnType.base[org.joda.time.Period, Interval](
      { period =>
